...
function(name) {
	...
	hitTest: function(point, options) {
		...
		if(this.name=='raster')
		{
			...
			var mtx = new Matrix();
			if(!this._children && !this.getBounds(mtx)
				.expand(options.tolerance)._containsPoint(point))
				return null;
		}
		else if (!this._children && !this.getRoughBounds()
				.expand(options.tolerance)._containsPoint(point))
			return null;
...
var Raster = this.Raster = PlacedItem.extend({
	...
	_hitTest: function(point, options, matrix) {
		var invMat = matrix ? matrix.createInverse() : null;
		var pt = invMat ? invMat.transform(point) : point;
		if (pt.isInside(this._getBounds().expand(options.tolerance))) {
			var that = this;
